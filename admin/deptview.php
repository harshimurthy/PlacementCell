...
<div class="deptdiv">
<form action="deptview.php" method="post">
	<h2 align=center>Display Department Coordinators Details</h2>
	<table class="delbox">
...
if((isset($_POST['usn']) || isset($_POST['branch'])) && isset($_POST['usnbutton']))
{
	...
	if($res_run=mysql_query($result))
	{
		...
		else
		{
			...
			$branch1=substr($branch,0,count($branch)-6);
			
			echo "<br/> USERNAME:  ".$username."<br/> ORIGINAL PASSWORD:  ".$pwd;
			echo "<br/> BRANCH: ".$branch1."<br/> PHONE:  ".$phone."<br/> EMAIL ID:  ".$email;
...
<div class="tabdiv">
<table class="viewtable"><tr> <td colspan=5>ALL EXECUTIVE STAFF ENTRIES</td></tr>
<tr><th>USERNAME</th><th>ORIGINAL PASSWORD</th><th>BRANCH</th><th>PHONE NUMBER</th><th>EMAIL ID</th></tr>

<?

$q="select * from `origpwd` where `branch` LIKE '%coord' order by `username` ASC";
if($res_run=mysql_query($q))
{
	for($i=0;$i<mysql_num_rows($res_run);$i++)
	{
			...
			$branch=mysql_result($res_run,0,'branch');
			$branch1=substr($branch,0,count($branch)-6);
			echo "<tr><td> ".$username."</td><td>".$pwd."</td><td> ".$branch1;
