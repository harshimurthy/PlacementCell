...
<form action="companydel.php" method="post">
	<h2 align=center>Delete A Company Entry</h2>
	<h6>Note: Deleting A company Entry will delete all the information associated with the company</h6>
	<table class="delbox">
	...
	if($res_run=mysql_query($result))
	{
		...
		else
		{
			...
			{	
					...
					mysql_query("delete from `company` where `username`='$usn'");
					mysql_query("drop table $usn");
					echo "All information associated with the company successfully deleted";
			}
			else
				echo "	Could not Delete! An error Occured";
		}
